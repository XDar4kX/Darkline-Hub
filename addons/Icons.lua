--[[
    ...
    Usage:
        ...
        local id = Icons:Get("heart")

        -- fuzzy (returns best match, even with typos)
        local id = Icons:Fuzzy("swrd")

        -- search (list of matches)
        local list = Icons:Search("arrow", 10)          -- up to 10 names

        -- everything (for dropdowns)
        local all = Icons:All()
]]
...
-- ICON_DATA is everything below (generated from lucide)
local ICON_DATA = {
...
-- index flat for iteration
local NAMES = {}
for k in pairs(ICON_DATA) do NAMES[#NAMES+1] = k end
table.sort(NAMES)

-- exact lookup, normalizes "Sword Icon" / "sword_icon" / "sword icon" -> "sword-icon"
local function norm(s)
    if type(s) ~= "string" then return "" end
    s = s:lower():gsub("[_%s]+", "-"):gsub("[^%w%-]", "")
    return s
end
...
-- fuzzy: returns best match using simple subsequence scoring
local function scoreMatch(needle, hay)
    -- hay contains needle as substring = highest
    if hay == needle then return 1000 end
    local idx = hay:find(needle, 1, true)
    if idx then return 500 - idx - math.abs(#hay - #needle) end
    -- subsequence score
    local s, n = 1, 1
    local score = 0
    while n <= #needle and s <= #hay do
        if needle:sub(n, n) == hay:sub(s, s) then
            score = score + (10 - math.min(s - n, 9))
            n = n + 1
        end
        s = s + 1
    end
    if n > #needle then return score end
    return -1
end

function Icons:Search(query, limit)
    limit = limit or 20
    local needle = norm(query)
    if #needle == 0 then return {} end

    local scored = {}
    for _, name in ipairs(NAMES) do
        local sc = scoreMatch(needle, name)
        if sc > 0 then
            scored[#scored+1] = { name = name, score = sc, id = ICON_DATA[name] }
        end
    end
...
-- just the best match id (or nil), handy for feeding ImageLabel.Image
function Icons:Fuzzy(query)
    local r = self:Search(query, 1)
    return r[1] and r[1].id or nil
end

-- flat list of all names (for dropdowns)
function Icons:All()
    return NAMES
end

-- raw data (if someone wants direct access)
function Icons:Raw() return ICON_DATA end

-- make Icons("foo") work as shorthand for Icons:Get("foo")
setmetatable(Icons, { __call = function(self, name) return self:Get(name) end })
